...
#Ex: python3 ../../data_scripts/graph_h5.py 2_exp inputdata.h5

# mandatory arguments
parser.add_argument("experimentName", help="Ex: 2_exp or 2_gauss, determines which values to use for nominal", type=str)
...
def main():
    ...
    for obs_num in range(len(obs_names)):
        plt.figure() 
        plt.plot([], [], ' ', label='\n'.join([vals.pnames[i] + '=' + str(round(vals.targets[i],2)) for i in range(len(vals.pnames))]))
        plot_target(obs_num)
        plt.title(args.experimentName + ' target of observable function: ' + vals.func_strs[obs_num])
        plt.xlabel("x")
        plt.ylabel("Number of Events")
        plt.legend()
        plt.savefig("h5_graphs/"+obs_names[obs_num] + "_target.pdf", format="pdf")
        plt.close()
        ...
        for run_num in range(num_runs):
            ...
            #params in legend, maybe keep
            plt.plot([], [], ' ', label='\n'.join([vals.pnames[i] + '=' + str(round(f['params'][run_num,i],2)) for i in range(len(vals.pnames))]))
